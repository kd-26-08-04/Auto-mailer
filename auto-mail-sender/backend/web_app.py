...
def _start_inline_worker() -> None:
    ...
    def worker_loop():
        import time
        interval = int(os.environ.get("WORKER_INTERVAL", "60"))
        print(f"[sequence-worker] Started — interval={interval}s. Email sequences will keep sending even when browser is closed.")
        while True:
            try:
                process_due_sends(tracking_base_url=TRACKING_BASE_URL, max_per_run=30)
            except Exception as exc:
                # Never crash — log and continue next cycle
...
@app.before_request
def check_login():
    # Let Flask handle OPTIONS preflight — after_request_cors adds the CORS headers
...
# These ensure a single failing request never crashes the server process.
